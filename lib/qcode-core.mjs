...
//
// `business-context.md` is the one file under `skills/` that inverts the rule: it's compass-check's
// own answers, not compass-check's own logic — the team fills it in once `generate` creates it, and
// from that point on it's exactly as project-owned as the board or the backlog. Excluding it from
// the manifest entirely (an earlier version of this function did) was too broad a hammer: that also
// hid it from `generate`, which has nothing to protect yet and NEEDS to render it once, the same as
// every other file a brand-new project doesn't have.
const FRAMEWORK_OWNED_PREFIXES = ['skills/', 'githooks/', 'ci/', 'scripts/', 'cockpit/'];
const FRAMEWORK_OWNED_EXACT = ['gitattributes'];
const PROJECT_OWNED_EXCEPTIONS = ['skills/compass-check/business-context.md'];

export function isFrameworkOwned(templateRelPath) {
  const p = templateRelPath.replace(/\\/g, '/');
  if (PROJECT_OWNED_EXCEPTIONS.includes(p)) return false;
 ...
 * NOT excluded (see the ownership comment above) — `generate` must render it like any other file;
 * what protects it from `sync` is its `owner: 'project'`, not absence from the manifest.
 */
